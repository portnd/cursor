...
import { useDashboardReflectiveStore } from "./DashboardReflectiveStore"
import { getColorDrawPriority } from "~/utils/colors"
import {
	IRoadConditionDetails,
	IRoadConditionList,
	IRoadConditionItem,
	IHighChart,
} from "~/core/modules/road/info/condition/infrastructure/RoadConditionModel"
...
import { useDashboardConditionStore } from "~/core/modules/dashboard/condition/store/DashboardConditionStore"
...
export const useDashboardRoadConditionStore = defineStore("dashboard/roadcondition", {
	state: (): IState => ({
		loading: false,
		map: null,
		params: {
		...
		conditionDataTable: [],
		conditionList: [],
		conditionGrade: JSON.parse(JSON.stringify(useInitData()?.conditionGrade())),
		conditionInput: [],
		refConditionGrade: [],
		allConditionInput: [],
	...
	actions: {
		async getRoads() {
			...
			if (roadId) {
				await this.getConditionList(roadId)
					.then(() => this.setDefaultParams())
					.then(() => this.initTypeRangeId())
		...
		getConditionType() {
			...
			const conditionType = dashboardConditionStore.conditionType
			...
			const matchedCondition = conditionOptions.find((option) => option.value === conditionType)
			const type = matchedCondition?.label as string
			this.params.condition_type = type

			this.checkConditionType(type)
		},
		setDaultConditionType() {
		...
		async getConditionList(roadId: number) {
			...
			this.loading = false
			console.log("getConditionList =", res.status)
			if (!res.status) {
		...
		async getConditionDetails() {
			console.log("❌ getConditionDetails =", this.params)
			if (this.params.id_parent && this.params.ref_condition_range_id) {
				this.loading = true

				this.details = {} as IRoadConditionDetails

				const req: { condition_range_type: string } = {
					condition_range_type: this.params.ref_condition_range_id?.toString(),
				}

				// const params = generateSearchParams(req)

				const service = new DashboardConditionService()
				const res = await service.getConditionDetails(this.params.id_parent, req)
		...
		createCriteriaCheckbox() {
			...
			this.params.criteria_id = options.length > 0 ? options.map((item) => item.value) : []
		...
		async createDataTable() {
			...
			console.log("createDataTable =", params.condition_type)
			...
			console.log("✅createDataTable conditionItems=", conditionItems)
			console.log("✅createDataTable generatedData=", generatedData)
			console.log("createDataTable conditionDataTable=", this.conditionDataTable)
		...
		processDataTable(conditionItems: IRoadConditionItem[], masterRule: ISurveyRule) {
			console.log("processDataTable =", conditionItems)
			...
			const processData = (items: IRoadConditionItem[], surveyType: string) => {
				...
				return items
					...
					.map((parent, index) => {
						...
						return {
							id: this.params.ref_condition_range_id === 3 ? index : `parent${index}`,
			...
			return [...processData(conditionItems, "AC"), ...processData(conditionItems, "CC")]
		},
		...
		createLine() {
			...
			if (longdo) {
				// Sort ascending by color priority so bad-condition (red) segments are drawn last
				// and always appear on top of good-condition (green) segments.
				const sorted = [...geoms].sort(
					(a, b) => getColorDrawPriority(a.color) - getColorDrawPriority(b.color),
				)

				const polylines = sorted.flatMap((geom) => {
		...
		graph() {
			...
			const graph = {
              ...
              `
				...
				// 		zoneAxis: "y",
				// 		zones: [
				// 			{ value: 2, color: "#42d235" },
				// 			{ value: 3, color: "#a4fca5" },
				// 			{ value: 4, color: "#f77a14" },
				// 			{ value: 99, color: "#ff290a" },
				// 		],
				// 		animation: false,
				// 	},
				// ],
		...
		onUpdateOwner() {
			...
			if (conditionStore.conditionType === 5) {
				dashboardReflectiveStore.params.owner_id = dashboardReflectiveStore.params.owner_id
					? dashboardReflectiveStore.params.owner_id
					: useInitData()?.reflectivityGrade()?.[0]?.id ?? null
			}

			dashboardCondition.params.owner_id = dashboardCondition.params.owner_id
