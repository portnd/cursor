import {
	...
	// DashboardSurfaceService,
	DashboardConditionService,
	IHighChart,
} from "../infrastructure"
...
export const useDashboardReflectiveStore = defineStore("dashboard/reflective", {
	state: (): IState => {
		return {
		loading: false,
		params: {
	...
	actions: {
		async getRoads() {
			...
			if (roadId) {
				this.roadId = roadId
				await this.getReflectivityList(roadId).then(() => this.setDefault())
				await this.getReflectivityDetails()
					.then(() => this.createCriteriaCheckbox())
					.then(() => this.createDataTable())

				await this.getLineList(roadId)

				this.oldRoadId = roadId
			}
		},
		setMap(map: any) {
			this.map = map
		...
		async getReflectivityList(roadId: number) {
			...
			if (dashboardConditionStore.conditionType === 5) {
				const service = new DashboardConditionService()
		...
		async setDaultConditionType() {
			...
			// this.params.ref_reflectivity_range_id =
			// 	this.reflectList.length > 0 ? this.reflectList[0].items[0].id : null

			if (!this.reflectList.length) {
				// this.params.id_parent = this.getLineOptions.length > 0 ? this.getLineOptions[0]?.value : null
				const dashboard = useDashboardStore()
				const roadId = Number(dashboard.params.road_id[0])
				await this.getReflectivityList(roadId).then(() => {
					this.params.ref_reflectivity_range_id = this.reflectList.length > 0 ? this.reflectList[0].items[0].id : null
				})
				...
				const graph = {
					...
					dataLabels: {
						...
						formatter: function (value: any, opt: any) {
							return `${value} กม. (${percent[opt.dataPointIndex]?.toFixed(2) ?? "0.00"}%)`
						},
						style: {
							position: "absolute",
							fontSize: "12px",
