...
import { useDashboardConditionCompareStore } from "../store"
// import { useConditionStore } from "~/core/modules/road/info/condition/store"

const store = useDashboardConditionCompareStore()
...
// 	() => [props.reloadData],
// 	async (_) => {
// 		await store.getLaneList()
// 		await store.getConditionList()
// 	}
// )
			...
			chart: {
				...
				toolbar: {
					...
					tools: {
						...
						customIcons: [
							{
								icon: '<button class="btn btn-outline rounded-3 apxchart-button-box">รีเซ็ต</button>',
								title: "รีเซ็ต",
