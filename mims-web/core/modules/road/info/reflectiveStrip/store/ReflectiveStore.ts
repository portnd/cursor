...
export const useReflectiveStore = defineStore("reflective", {
	state: (): IState => ({
		loading: false,
		params: {
			graph_type: "graph",
	...
	actions: {
		...
		async getReflectivityList(roadId: number) {
			this.loading = true
			...
			this.reflectList = Array.isArray(res.data) ? res.data : []
			// setDefault ต้องรันทันทีหลัง reflectList ถูก set เสมอ เพื่อป้องกัน race condition
				try {
					this.setDefault()
				} catch (e) {
					console.error("[ReflectiveStore] setDefault error:", e)
				}
			}
		},
		setDefault() {
			...
			// Fallback to first available range type from master list when no owner config exists
			const refReflectivityRangeList = initData?.refReflectivityRange() ?? []
			// อ่านจาก state โดยตรง เพื่อหลีกเลี่ยง getter ที่อาจเห็น state เก่าเมื่อ double-mount (log พิสูจน์แล้วว่า getYearOptions คืน [] ทั้งที่ reflectList.length === 1)
			const list = this.reflectList ?? []
			const firstYear = list.length > 0 ? list[0]?.year ?? null : null
			const firstYearItems = firstYear != null ? list.find((r) => r.year === firstYear)?.items ?? [] : []
			this.params.year = firstYear
			this.params.id_parent = firstYearItems.length > 0 ? firstYearItems[0]?.id_parent ?? null : null
			...
			this.params.ref_reflectivity_range_id =
				refReflectivityGrade.length > 0
					? refReflectivityGrade[0]?.ref_reflectivity_range_id
					: (refReflectivityRangeList.length > 0 ? refReflectivityRangeList[0]?.id ?? null : null)
		...
		setReflectInput() {
			...
			const allReflectivityInput = defaultData.flatMap((parent) => {
				if (!parent?.id || !parent?.road_line) return []
				...
				reflectRule.forEach((rule) => {
					...
					if (rule.id === parent.id) {
						...
						rule.white.reflectivity_list.forEach((whiteRule, index) => {
							...
							if (white && whiteRule.grade_id === white.grade?.id) {
		...
		async createDataTable() {
			...
			const filtered = generatedData.filter((item) => {
				return params.ref_reflectivity_range_id === 2 || (item?.child && item?.child?.length > 0)
			})
			this.reflectivityDataTable = filtered
		...
		async callBackUpdateData(roadId: number, method: string) {
			...
			await getReflectivityList(roadId)
	...
	getters: {
		getYearOptions(state) {
			...
			const list = reflectList ?? []
			const options = list.map((item) => ({ label: `${item.year + 543}`, value: item.year }))
		...
		getLineOptions(state) {
			...
			const list = reflectList ?? []
			const items = list.find((item) => item.year === params.year)?.items ?? []
