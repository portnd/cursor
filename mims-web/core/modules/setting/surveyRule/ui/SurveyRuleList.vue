...
const headers: THeader[] = [
	...
	{ text: "ชื่อเกณฑ์การจำแนกสภาพทาง", value: "name", width: 125 },
...
// เพิ่มข้อมูล
const createItem = () => {
	return navigateTo(`/settings/survey-rules/create`)
}

// แก้ไขข้อมูล
const editItem = (item: ISurveyRuleList) => {
	return navigateTo(`/settings/survey-rules/${item.id}/edit`)
}
...
const onSearch = async () => {
	await dataTable.value.searchData({
		name: search.name,
		ref_condition_range_id: search.ref_condition_range_id,
	})
}

// watch(
// 	() => search.ref_condition_range_id,
// 	() => {
// 		onSearch()
// 	}
// )
...
const resetSearch = async () => {
	await dataTable.value.searchData({
		name: "",
		ref_condition_range_id: undefined,
	})
	search.name = ""
	search.ref_condition_range_id = 0
}
...
<template>
	<form class="row mb-3" @submit.prevent="onSearch" @keyup.enter.prevent="onSearch">
		<div class="col-md-3 col-12 mb-2">
			<VTextInput v-model="search.name" label="ชื่อเกณฑ์การจำแนกสภาพทาง" name="name" @keyup.enter="onSearch" />
		</div>
		<div class="col-md-3 col-12 mb-2">
			<VSelect
				v-model="search.ref_condition_range_id"
		...
		<div class="col-12 col-md-6 d-flex justify-content-between align-items-end">
			<div class="mb-2 d-flex align-self-end">
			...
			<div class="mb-2 order-first order-md-last">
	...
	<div class="row">
		<div class="col-xl-12">
			<ServerSideDataTable ref="dataTable" :headers="headers" url="/settings/owners">
				<!-- begin::Items -->
				...
				<template #item-type="{ item }">
					<div class="text-center">{{ item.ref_condition_range.name }}</div>
				</template>
