...
const onSubmit = handleSubmit(async (_, actions) => {
	useAction(actions)
	const res = await store.editAsset("in")
	if (res?.status) {
							...
							<div class="row">
								...
								<div class="col-12 mb-3 disabled">
									<GeomTypeForm
										...
										@update:line-color-code="($colorCode) => (store.asset.line_color_code = $colorCode)"
										@update:image="($image) => (store.imageAsset = $image)"
										@update:delete-image-default="($image) => (store.asset.icon_filepath = $image)"
									/>
								</div>
							</div>
						</div>
