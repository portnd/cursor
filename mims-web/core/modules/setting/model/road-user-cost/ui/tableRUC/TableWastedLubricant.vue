					...
					<table class="table customize-basic-table mb-0">
						<thead>
							<tr>
								...
								<th class="text-center fw-semibold">OILCONT <span class="required"></span></th>
								<th class="text-center fw-semibold">OILOPER <span class="required"></span></th>
							</tr>
						</thead>
						<tbody v-for="(item, name, index) in rucStore.rucData" :key="index">
							<tr>
								...
								<td><VNumberInput v-model="item.oil_cont" :name="`oil_cont_${index}`" :precision="4" /></td>
								<td><VNumberInput v-model="item.oil_oper" :name="`oil_oper_${index}`" :precision="4" /></td>
							</tr>
						</tbody>
					</table>
				</div>
