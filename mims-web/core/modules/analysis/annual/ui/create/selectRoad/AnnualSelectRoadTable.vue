...
// onMounted(() => {
// 	if (Object.keys(store.copyData).length > 0) {
// 		store.selectedPrepareData = store.prepareData.prepare_data
// 		store.prepareDataId = store.selectedPrepareData.filter((item) => item.is_selected).map((item) => item.id)
// 	}
// })

// watch(
// 	() => store.prepareData.prepare_data,
// 	(newPrepareData) => {
// 		if (newPrepareData.length > 0) {
// 			store.prepareDataId = newPrepareData.map((item) => item.id)
// 		}
// 	}
// )
