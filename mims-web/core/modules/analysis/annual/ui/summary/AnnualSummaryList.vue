...
import { IReportParams } from "../../../strategic/infrastructure"
import { useAnnualSummaryDashboardStore, useAnnualAnalyzeSummaryStore, useAnnualAnalyseEditStore } from "../../store"
import { DISPLAY_IRI, DISPLAY_METHOD } from "../../../strategic/store"
import AnnualSummaryGraph from "./AnnualSummaryGraph.vue"
import AnnualSummaryPlanTable from "./AnnualSummaryPlanTable.vue"
...
const summary = useAnnualAnalyzeSummaryStore()
const editStore = useAnnualAnalyseEditStore()
...
onMounted(async () => {
	...
	editStore.$reset()
	await store.getDashboardData(id)
...
// 	const res = await store.createFavorite(id)
	...
	if (res?.status) {
		useHandlerSuccess(res.code, {
			...
			fn: () => {
				...
				navigateTo(`/analyses/annual/select-road/${res.data.id}`)
			},
		})
	}
}
...
const downloadFile = (fileType: string) => {
	...
	const requestUrl = `analyze/${id}/report/report${store.exportReport.reportTypeNo}${keyword !== "" ? "?" + keyword : ""}`
...
watch(
	...
	() => {
		if (store.defaultData) {
			favorite.value = store.defaultData.is_favorite
		}
	}
)
...
const updateWidth = () => {
	...
	const element = document.querySelector("#content") as HTMLElement
	const topEnd = document.querySelector(".top-end") as HTMLElement
	const isMobile = ref()
...
<template>
	...
	<div class="row">
		<div id="content" class="col-12" :class="!mapShow.collapsed ? 'col-lg-8 col-xl-7' : 'col-lg-12 col-xl-12'">
			<div class="card p-5 mb-5">
				<div class="row mb-md-5">
					<div class="col-lg-5 col-md-4 col-6 align-self-center">
						<!-- <button
							type="button"
					...
					<div class="col-lg-7 col-md-8 col-6 d-flex align-items-center justify-content-end gap-4 text-md-end">
						<span class="dropdown">
							<button
							...
							<ul class="dropdown-menu lh-2 mb-0" aria-labelledby="dropdownMenuButton1">
								<li v-for="(item, index) of basedMenu" :key="index">
									<NuxtLink
										class="dropdown-item fs-6 py-2 cursor-pointer"
						...
						<ul class="dropdown-menu lh-2 mb-0 py-1 p-4 list-menu" aria-labelledby="list-menu-1">
							<li
								v-show="canEdit"
								class="px-4 py-2"
								@click="navigateTo(`/analyses/annual/summary/${id}/${path}/detail`)"
							>
								รายละเอียด
							</li>
							<li v-show="canEdit" class="px-4 py-2" @click="navigateTo(`/analyses/annual/select-road/${id}/edit`)">
								แก้ไข
							</li>
						...
						<!-- <NuxtLink
							:to="`/analyses/annual/summary/${id}/budget-limit/detail`"
						...
						<NuxtLink
							v-show="canEdit"
							:to="`/analyses/annual/select-road/${id}/edit`"
							class="btn btn-outline btn-outline-primary rounded-4 ms-3 mb-3 mb-md-0 px-8 py-3 fw-semibold"
						>
							แก้ไข
						</NuxtLink>
		...
		<div id="map" class="col-12 map-sticky" :class="!mapShow.collapsed ? 'col-lg-4 col-xl-5' : 'map-collapsed'">
			<div class="widget">
				<KeepAlive>
					<VMap v-model="mapShow" :loading="summary.loading" height="97vh" :is-sticky="true" @map="store.setMap">
						<template #top-end>
							...
							<div v-if="isModalMap" class="map-layer rounded-2 shadow">
								...
								<div class="map-detail px-4 pb-8 pt-2">
									<VSelect
										...
										:can-clear="false"
										:can-deselect="false"
										@update:model-value="store.getMapData(id)"
									/>
									<VSelect
										v-if="store.map_params.display === DISPLAY_IRI"
										v-model="store.map_params.criteria"
							...
							<div class="position-relative">
								...
								>
									...
									<div
										...
										:style="{
											width:
												store.map_params.display !== DISPLAY_METHOD
													? '184px'
										...
										:style="{
											left:
												store.map_params.display !== DISPLAY_METHOD
													...
													? '100px'
													: '75px',
											width: store.map_params.display !== DISPLAY_METHOD ? '160px' : '80px',
										}"
									>
									...
									<div
										...
										"
										:style="{
											left: store.map_params.display !== DISPLAY_METHOD ? '110px' : `${store.getSymbolPositionLeft}px`,
										}"
									>
									...
									>
										<div
											...
											style="width: 205px"
										>
											<template v-for="(item, index) in store.getCriteriaGradeAC" :key="index">
												<div class="row">
											...
											<div class="parent">
												<template v-for="(item, index) in store.getCriteriaLegend" :key="index">
													<div class="row d-flex">
														<div class="col-2 square mb-3" :style="{ 'background-color': item.color }"></div>
														<div
															class="col-10 mb-3 text-gray-700"
															:class="store.getCriteriaLegend.length > 5 ? ' criteria-ellipsis-two' : ''"
														>
															{{ item.name }}
														</div>
													</div>
									...
									>
										<div style="height: 0px; width: 184px; border: 0.5px #fdb833 solid"></div>
										<div
											style="
												height: 16px;
												width: 160px;
										...
										>
											<div class="row ms-0 d-flex align-items-center" style="width: 205px">
												<template v-for="(item, index) in store.getCriteriaGradeCC" :key="index">
													<div class="row">
														<div class="col-2 square mb-3" :style="{ 'background-color': item.color }"></div>
														<div class="col-10 mb-2 text-gray-700 criteria-ellipsis-one" style="font-size: 10px">
															{{ item.name }}
