...
import { useChildRoadConditionSummaryReportStore } from "../../store"
import { IValidate } from "~/core/shared/types/Validate"
const store = useChildRoadConditionSummaryReportStore()
const submitNum = ref()

onMounted(async () => {
	handleReset()
	await store.getChildRoadConditionSummaryFilter()
})
...
const onSubmit = handleSubmit((_, actions) => {
	...
	useDownloadFile(
		...
		`/report/type7?year=${store.data.year}&road_section_id=${store.data.road_section_id}&type=${store.data.type}&factor=${store.data.condition}&filter_criteria_id=${store.data.criteria_id}`
	)
})
