...
import { useChildAssetMapReportStore } from "../../store"
import { IValidate } from "~/core/shared/types/Validate"
const store = useChildAssetMapReportStore()
const submitNum = ref()

onMounted(async () => {
	handleReset()
	await store.getChildAssetMapFilter()
})
...
const onSubmit = handleSubmit((_, actions) => {
	useAction(actions)
	useDownloadFile(
		"รายงานแผนที่สินทรัพย์",
		`/report/type2?road_section_id=${store.data.road_section_id}&asset_id=${store.data.asset_id}&type=${store.data.type}`
	)
})
	...
	<div class="row">
		<div class="col-12">
			<VSelect
				...
				:options="toOptions(store.dataChildAssetMapFilter.filter_road)"
		...
		<div class="col-12">
			<VSelect
				...
				:options="toOptions(store.dataChildAssetMapFilter.filter_asset)"
