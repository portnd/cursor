...
import { useReportMaintenanceReportStore } from "../../store"
import { IValidate } from "~/core/shared/types/Validate"

const submitNum = ref()
const store = useReportMaintenanceReportStore()

onMounted(async () => {
	handleReset()
	await store.getReportMaintenanceFilter()
})
...
const onSubmit = handleSubmit((_, actions) => {
	...
	useDownloadFile(
		...
		`/report/type12?year=${store.data.year}&road_section_id=${store.data.road_section_id}&filter_condition_name=${store.data.condition_name}&type=${store.data.type}`
	)
})
...
<template>
	...
	<div class="row">
		<div class="col-12">
			<VSelect
				v-model="store.data.condition_name"
				:options="store.getConditionOptions"
				label="เงื่อนไขการออกรายงาน"
				name="condition"
				placeholder="เลือก"
				@update:model-value="
					() => {
						store.data.year = null
