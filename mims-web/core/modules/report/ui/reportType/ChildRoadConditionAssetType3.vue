...
import { useChildRoadConditionAssetReportStore } from "../../store"
import { IValidate } from "~/core/shared/types/Validate"
const store = useChildRoadConditionAssetReportStore()
const submitNum = ref()

onMounted(async () => {
	handleReset()
	await store.getChildRoadConditionAssetFilter()
})
...
const onSubmit = handleSubmit((_, actions) => {
	...
	useDownloadFile(
		"รายงานสรุปสินทรัพย์",
		`/report/type3?road_section_id=${store.data.road_section_id}&type=${store.data.type}`
	)
})
	...
	<div class="row">
		<div class="col-12">
			<VSelect
				...
				:options="toOptions(store.dataChildRoadConditionAssetFilter.filter_road)"
