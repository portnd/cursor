...
import { useChildRoadDamageReportStore } from "../../store"
import { IValidate } from "~/core/shared/types/Validate"
const store = useChildRoadDamageReportStore()
const submitNum = ref()

onMounted(async () => {
	handleReset()
	await store.getChildRoadDamageFilter()
})
...
const onSubmit = handleSubmit((_, actions) => {
	...
	useDownloadFile(
		...
		`/report/type10?year=${store.data.year}&road_section_id=${store.data.road_section_id}&type=${store.data.type}`
	)
})
