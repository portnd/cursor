...
import { useReportVolumeAadtReportStore } from "../../store"
import { IValidate } from "~/core/shared/types/Validate"

const submitNum = ref()
const store = useReportVolumeAadtReportStore()

onMounted(async () => {
	handleReset()
	await store.getReportVolumeAadtFilter()
})
...
const onSubmit = handleSubmit((_, actions) => {
	...
	useDownloadFile(
		"รายงานปริมาณจราจร",
		`/report/type14?year=${store.data.year}&road_section_ids=${store.data.road_section_id}&type=${store.data.type}`
	)
})
