...
import { useChildRoadSummaryReportStore } from "../../store"
import { IValidate } from "~/core/shared/types/Validate"
const store = useChildRoadSummaryReportStore()
const submitNum = ref()

onMounted(async () => {
	handleReset()
	await store.getChildRoadSummaryFilter()
})
...
const onSubmit = handleSubmit((_, actions) => {
	...
	useDownloadFile(
		...
		`/report/type4?road_group_ids=${store.data.road_group_ids}&type=${store.data.type}`
	)
})
...
<template>
	...
	<div class="row">
		<div class="col-12">
			<VSelect
				v-model="store.data.road_group_ids"
				:options="toOptions(store.dataChildRoadSummaryFilter.filter_road)"
