...
import { useChildRoadDamageSummaryReportStore } from "../../store"
import { IValidate } from "~/core/shared/types/Validate"
const store = useChildRoadDamageSummaryReportStore()
const submitNum = ref()

onMounted(async () => {
	handleReset()
	await store.getChildRoadDamageSummaryFilter()
})
...
const onSubmit = handleSubmit((_, actions) => {
	...
	useDownloadFile(
		...
		`/report/type11?year=${store.data.year}&road_section_id=${store.data.road_section_id}&type=${store.data.type}`
	)
})
