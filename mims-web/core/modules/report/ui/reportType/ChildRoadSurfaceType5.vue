...
import { useChildRoadSurfaceReportStore } from "../../store"
import { IValidate } from "~/core/shared/types/Validate"
const store = useChildRoadSurfaceReportStore()
const submitNum = ref()

onMounted(async () => {
	handleReset()
	await store.getChildRoadSurfaceFilter()
})
...
const onSubmit = handleSubmit((_, actions) => {
	...
	useDownloadFile(
		...
		`/report/type5?year=${store.data.year}&road_section_id=${store.data.road_section_id}&type=${store.data.type}`
	)
})
