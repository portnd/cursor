...
import { useChildRoadReflectLightReportStore } from "../../store"
import { IValidate } from "~/core/shared/types/Validate"
const store = useChildRoadReflectLightReportStore()
const submitNum = ref()

onMounted(async () => {
	handleReset()
	await store.getChildRoadReflectLightFilter()
})
...
const onSubmit = handleSubmit((_, actions) => {
	...
	useDownloadFile(
		...
		`/report/type8?year=${store.data.year}&road_section_id=${store.data.road_section_id}&filter_criteria_id=${store.data.km}&type=${store.data.type}`
	)
})
...
<template>
	...
	<div class="row">
		...
		<div class="col-12">
			<VSelect
				v-model="store.data.km"
				:options="store.getKmOptions"
				label="ช่วง กม. สำหรับเฉลี่ยข้อมูล"
				name="km"
				:required="true"
				placeholder="เลือก"
			/>
