...
import { useChildRoadConditionReportStore } from "../../store"
import { IValidate } from "~/core/shared/types/Validate"
const store = useChildRoadConditionReportStore()
const submitNum = ref()

onMounted(async () => {
	handleReset()
	await store.getChildRoadConditionFilter()
})
...
const onSubmit = handleSubmit((_, actions) => {
	...
	useDownloadFile(
		"รายงานข้อมูลสภาพทาง",
		`/report/type6?year=${store.data.year}&road_section_id=${store.data.road_section_id}&distance=${store.data.km}&type=${store.data.type}`
	)
})
