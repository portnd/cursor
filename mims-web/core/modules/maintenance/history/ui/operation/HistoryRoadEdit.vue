...
import { useMaintenanceHistoryRoadsEditStore } from "../../store"
import { IValidate } from "../../../../../shared/types/Validate"

const store = useMaintenanceHistoryRoadsEditStore()
useStoreLifecycle(store)

const route = useRoute()
const itemId = Number(route.params.roadId)
...
onMounted(async () => {
	...
	await store.getMaintenanceRoadInfo(parentId, itemId)
})
...
const onSubmit = handleSubmit(async (_, action) => {
	...
	if (store.defaultData.maintenance_type === 2 && store.gridNo === null) {
		...
		const res = await store.updateMaintenanceRoadInfo(parentId, itemId)
	...
	<VSkeletonLoader :loading="store.loading">
		<div class="row card p-5 py-8">
			...
			<form class="row pe-0" @submit="onSubmit">
				...
				<div class="col-md-4 col-12 mt-4">
					<VNumberInput
						v-model="store.defaultData.distance"
						label="ระยะทาง (กม.)"
						name="distance"
						:precision="3"
						:disabled="true"
