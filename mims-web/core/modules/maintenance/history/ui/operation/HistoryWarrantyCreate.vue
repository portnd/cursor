...
import { useMaintenanceWarrantyCreateStore } from "../../store"
import { IValidate } from "../../../../../shared/types/Validate"

const store = useMaintenanceWarrantyCreateStore()
...
const onSubmit = handleSubmit(async (_, action) => {
	...
	if (store.maintenance_type === 2 && store.grid_no === null) {
		...
		const res = await store.createMaintenanceWarrantyInfo(id)
