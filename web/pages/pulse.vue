...
import { usePulseApi } from '~/core/modules/pulse/infrastructure/pulse-api'
...
async function loadHiddenUserIds() {
  if (!isCEO.value) {
    hiddenUserIds.value = []
    return
  }
  loadingHiddenUsers.value = true
  try {
    const res = await getHiddenUsers()
    hiddenUserIds.value = Array.isArray(res.user_ids)
      ? res.user_ids.filter((v) => Number.isInteger(v) && v > 0)
      : []
  } catch {
    hiddenUserIds.value = []
  } finally {
    loadingHiddenUsers.value = false
  }
}

async function onToggleUserVisibility(payload: { userId: number; hidden: boolean }) {
  if (!isCEO.value || loadingHiddenUsers.value) return
