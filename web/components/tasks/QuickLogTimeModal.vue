        ...
        <div class="relative w-full max-w-lg bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl shadow-black/50 flex flex-col max-h-[90vh]">
          ...
          <div class="flex-1 overflow-y-auto px-5 py-4 space-y-4">
            ...
            <div>
              ...
              <!-- Task Dropdown Results -->
              <Transition name="dropdown">
                <div
                  v-if="showDropdown && (filteredTasks.length || loadingTasks || (teamTasks.length === 0 && !loadingTasks))"
                      ...
                      <div class="flex-1 min-w-0">
                        ...
                        <div class="flex items-center gap-2 mt-1">
                          ...
                          <span
                            ...
                            class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-medium text-gray-300 bg-gray-700/80"
                          >
                            <span
                              v-if="task.project_color"
                          ...
                          <span
                            ...
                            class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-medium bg-indigo-900/60 text-indigo-300 border border-indigo-700/40"
                          >
                          ...
                          <span
                            v-else
                            class="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] font-medium bg-gray-700/60 text-gray-500"
                          >
                            Unassigned
                          </span>
                        </div>
              ...
              <div
                v-if="selectedTask"
                class="mt-2 flex items-center gap-3 p-3 bg-purple-900/20 border border-purple-700/40 rounded-xl"
              >
                ...
                <div class="flex-1 min-w-0">
                  ...
                  <div class="flex items-center gap-2 mt-1">
                    <span
                      v-if="selectedTask.project_name"
                      class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] text-gray-400 bg-gray-700/60"
                    >
                      <span
                        v-if="selectedTask.project_color"
                        class="w-1.5 h-1.5 rounded-full"
                        :style="{ backgroundColor: selectedTask.project_color }"
                      />
                      {{ selectedTask.project_name }}
                    </span>
                    <span
                      v-if="assigneeLabel(selectedTask)"
                      class="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] bg-indigo-900/60 text-indigo-300 border border-indigo-700/40"
                    >
                      <svg class="w-2.5 h-2.5" fill="currentColor" viewBox="0 0 20 20">
                        <path d="M10 9a3 3 0 100-6 3 3 0 000 6zm-7 9a7 7 0 1114 0H3z" />
                      </svg>
                ...
                <button
                  type="button"
                  @click="clearTask"
                  class="shrink-0 p-1 rounded-lg text-gray-500 hover:text-red-400 hover:bg-red-900/20 transition-colors"
                >
            ...
            <!-- Step 2: Time Entry (shown only after task is selected) -->
              ...
              <div v-if="selectedTask" class="space-y-3">
                <div>
                  ...
                  <div class="flex items-center gap-3">
                    ...
                    <div
                      v-if="totalMinutes > 0"
                      class="ml-auto text-xs text-purple-400 font-medium"
                    >
                ...
                <div>
                  <label class="text-xs font-medium text-gray-400 uppercase tracking-wide mb-2 block">Description <span class="normal-case text-gray-600">(optional)</span></label>
                  <input
                ...
                <div class="flex items-start gap-2 p-3 bg-blue-900/20 border border-blue-700/30 rounded-lg">
                  ...
                  <p class="text-xs text-blue-300/80 leading-relaxed">
                    ...
                    Ownership remains with {{ assigneeLabel(selectedTask) || 'the assignee' }}.
                  </p>
                </div>
              </div>
            </Transition>
...
async function loadTeamTasks() {
  ...
  try {
    teamTasks.value = await tasksApi.getTeamActiveTasks()
  } catch {
    teamTasks.value = []
  } finally {
...
async function submitLog() {
  ...
  try {
    await tasksApi.logTime(selectedTask.value.id, totalMinutes.value, logDescription.value.trim())
    emit('logged')
