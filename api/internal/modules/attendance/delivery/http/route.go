...
func RegisterRoutes(r *gin.RouterGroup, uc domain.AttendanceUsecase) {
	...
	g := r.Group("/attendance")
	{
		...
		g.POST("/offsite-check-in/request", h.requestOffsiteCheckIn)
		g.POST("/offsite-check-out/request", h.requestOffsiteCheckOut)
		...
		admin := g.Group("/admin")
		{
			...
			admin.GET("/offsite-check-in/pending", h.adminListPendingOffsiteCheckIn)
			admin.PATCH("/offsite-check-in/:id/review", h.adminReviewOffsiteCheckIn)
			admin.GET("/offsite-check-out/pending", h.adminListPendingOffsiteCheckOut)
			admin.PATCH("/offsite-check-out/:id/review", h.adminReviewOffsiteCheckOut)
			admin.GET("/leaves/pending", h.listPendingLeaves)
			admin.GET("/leaves", h.listAdminLeaves)
			admin.PATCH("/leaves/:id/review", h.reviewLeave)
			admin.PATCH("/leaves/:id", h.updateAdminLeave)
			admin.PATCH("/leaves/:id/cancel", h.cancelAdminLeave)
			admin.DELETE("/leaves/:id", h.deleteAdminLeave)
