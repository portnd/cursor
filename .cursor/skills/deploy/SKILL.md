...
name: deploy
description: >-
  Deploy — commit, push, and manage git operations with safety checks and rollback plans.
  Use when code is complete, needs to be pushed, or when the user mentions deploy, commit,
  push, git, release, or ship.
---
...
4. **Stage and commit**:
   - Stage the appropriate files (ASK USER if unsure about untracked files)
...
2. **Push execution**:
   - **ASK USER** — always confirm before push
...
1. **Always ask before pushing**: Never push without user confirmation.
...
9. **Auto-run safe commands**: git status, git diff, git fetch, git log, test runs — all auto-run.
10. **Ask before destructive commands**: git push, git reset, git rebase, git clean — ASK USER always.
