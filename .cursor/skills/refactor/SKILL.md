...
name: refactor
description: >-
  Refactor — restructure existing code to improve quality without changing behavior.
  Use when code works but structure is poor, duplicated, too long, or highly coupled,
  or when the user mentions refactoring, restructuring, cleaning up, or improving code structure.
---

# /refactor — Safe Code Refactoring Protocol

## Source-of-Truth Refactoring

- Read `requirements.md` or `PRD.md` before refactoring when the behavior is product-driven.
- Preserve the active source document's intended behavior exactly.
- If a refactor reveals a mismatch, stop and reconcile the source document before continuing.
...
## PRD-Aware Refactoring

- Read `PRD.md` first when the refactor is driven by product changes or scope changes.
- If behavior must change to satisfy a new PRD, treat it as a feature task, not refactoring.
- Keep the PRD updated when the refactor changes externally visible behavior.
