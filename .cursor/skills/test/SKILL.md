...
name: test
description: >-
  Testing — write tests, run test suites, and verify code correctness with coverage analysis.
  Use after code implementation, when writing tests, running test suites, measuring coverage,
  or when the user mentions testing, test coverage, unit tests, integration tests.
---
...
## Source-of-Truth Test Discovery

- Read `requirements.md` or `PRD.md` before designing tests when the behavior under test is product-driven.
- Use the active source document to derive acceptance criteria and edge cases.
- If the active source document changes, revisit the tests and update them to match.
...
4. **Install missing test dependencies** (if needed):
   - If no test framework found → ASK USER which framework to install
...
6. **If no test framework exists**: ASK USER which framework to set up before proceeding.
7. **Parallel execution**: When running multiple test commands, run them in parallel if safe.
8. **Auto-install**: If any tool/dependency is missing during testing, install it automatically. Only ask user for framework selection or major version decisions.
